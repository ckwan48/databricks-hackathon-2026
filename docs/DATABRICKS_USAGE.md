...
| **Model Serving — Llama 4 Maverick** | Powers **3 on-demand agents** (insights · reasoning · simulation), a **grounded copilot** that refuses to fabricate numbers, an **11-language translator**, and — through the *same* endpoint — **multimodal vision** that reads an uploaded photo and routes it to a specialty. | The "Explain" buttons, Ask-the-Data answers, the Translate + 🔊 Listen, and the Referral photo upload. |
