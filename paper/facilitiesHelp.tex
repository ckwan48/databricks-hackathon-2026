...
\smallskip
\noindent\textbf{Keywords:} healthcare facility data; trust and evidence grading; uncertainty quantification; medical deserts; supply--demand; causal inference; confounding; propensity scores; geometric deep learning; spatial accessibility; Databricks.
...
\item \textbf{nfhs\_5\_district\_health\_indicators}~\cite{nfhs5} --- $706$ districts $\times\,109$ indicators; powers the causal layer.
\end{itemize}
...
\paragraph{Entity resolution.} The same facility is scraped from many sources. We deduplicate to one canonical record per pre-computed match key \texttt{cluster\_id} with the window function {\small\ttfamily row\_number() OVER (PARTITION BY cluster\_id ORDER BY unique\_id)}, collapsing $10{,}088$ raw rows to $9{,}959$ facilities. A medallion pipeline (bronze$\to$silver$\to$gold) on a serverless SQL Warehouse then assembles the evidence text, counts independent source types, and joins facilities$\to$PIN$\to$district.
...
\item \textbf{Non-popularity.} A gradient-boosting classifier~\cite{friedman2001} predicting \textsf{STRONG} from facility \emph{metadata only} (capacity, doctors, web presence, \#specialties, facility type)---\emph{excluding} the rule inputs---scores only $4$-fold ROC-AUC $=0.57$, barely above chance. Therefore the grade reflects \emph{cited evidence, not size or fame.}
\end{enumerate}
We additionally expose a SHAP-style~\cite{lundberg2017} per-claim attribution (Fig.~\ref{fig:fi}) that decomposes each confidence score into its contributing signals.
...
\begin{enumerate}[leftmargin=1.4em,itemsep=1pt]
\item \textbf{Structure learning} --- the PC algorithm~\cite{spirtes2000} (constraint-based conditional-independence tests) infers a DAG (Fig.~\ref{fig:dag}), identifying which variables to adjust for.
...
\item \textbf{Double Machine Learning}~\cite{chernozhukov2018} --- partial out confounders from both treatment and outcome with ML and regress the residuals (Frisch--Waugh--Lovell) with cross-fitting, robust to nonlinear confounding.
\item \textbf{Propensity-score matching}~\cite{rosenbaum1983} --- match high- vs.\ low-$X$ districts on $\hat e(Z)=\Pr(X{=}1\mid Z)$ and compare outcomes.
\item \textbf{Sensitivity (E-value)}~\cite{vanderweele2017} --- the minimum strength an unmeasured confounder would need with both $X$ and $Y$ to nullify the effect; large $=$ robust.
\end{enumerate}
...
Beyond linear adjustment we ran: \textbf{GAM + quantile regression}~\cite{hastie1990,koenker1978} for nonlinear dose--response and high-burden-district (tail) effects (Fig.~\ref{fig:gam}); \textbf{multilevel/hierarchical} models~\cite{gelman2007} with empirical-Bayes shrinkage to stabilize small-sample districts; penalized regression with \textbf{stability selection}~\cite{meinshausen2010}; and \textbf{geometric deep learning}~\cite{kipf2017}---a spatial GNN with positional encoding and a Tobler's-Law~\cite{tobler1970} smoothing penalty---benchmarked against baselines (Fig.~\ref{fig:gnn}). Moran's $I$~\cite{moran1950} is used as a spatial-autocorrelation \emph{diagnostic}, not as proof of a spatial process.
...
Roadmap: 2SFCA accessibility scoring~\cite{luo2003} to weight reachable supply within a travel radius; facility-placement optimization (where to build to maximally shrink the evidenced gaps); and \emph{last-mile} integrations that turn a verdict into an action---open or route to a facility in Google Maps, place a call or WhatsApp message to it, and email a shortlist or referral letter to a colleague---so a trusted recommendation becomes a next step in one tap. Multimodal intake is already live: the app accepts a photo---a facility board, a prescription, or the condition itself (an infected eye, a swollen limb)---and routes it to the right specialty using the \emph{same} Llama~4 Maverick Model-Serving endpoint as the text agents. The assistant also answers in eleven languages with text-to-speech.

\section{Honesty, uncertainty, and limitations}
Every claim cites the facility's own text; confidence is always shown; the assistant refuses to invent numbers, answering only from grounded gold-table data. Limitations: NFHS-5 is \emph{cross-sectional} (one wave), so our adjusted associations are the strongest single-wave causal claim---two waves would enable difference-in-differences. We also cannot run a randomized controlled trial (an A/B test) on districts---one cannot randomly assign sanitation or schooling---so propensity-score matching serves as the \emph{observational analog} of randomized assignment, balancing covariates between ``treated'' and ``control'' districts; a true interventional test would require a field or natural experiment (e.g.\ a staged policy rollout). Scraped supply data carries visibility bias. Suppressed NFHS values are MNAR and are flagged, never imputed.

\section{Future work: temporal causation, forecasting, and connectivity}
\paragraph{From cross-sectional to temporal causation.} The single largest gain is a \emph{second wave}. Linking NFHS-4 (2015--16) to NFHS-5 (2019--21) at the district level turns our cross-section into a two-period \emph{panel}, enabling: (i) \textbf{difference-in-differences}---comparing the \emph{change} in outcomes where a lever (sanitation, schooling) rose against districts where it did not, which differences out time-invariant district confounders; (ii) \textbf{district fixed-effects panels} and \textbf{lagged / dynamic models}, so a lever at time $t$ predicts an outcome at $t{+}1$; and (iii) \textbf{event-study designs} around the timing of district-level policy rollouts.
\paragraph{Stronger identification.} Where assignment is plausibly as-good-as-random, \textbf{instrumental variables} and \textbf{regression-discontinuity} (around programme-eligibility thresholds) would sharpen causal claims, and \textbf{synthetic control} would estimate a district's counterfactual trajectory after a specific intervention.
\paragraph{Causal forecasting.} A temporal panel lets us move from explanation to \emph{prediction}: Bayesian structural time-series / causal-impact models to forecast district outcomes and the \emph{counterfactual} effect of a planned facility, plus demand forecasting so a planner sees where need is \emph{growing}, not only where it is today.
\paragraph{Connectivity and deployment.} Beyond today's Google-Maps / share last-mile and multimodal photo intake: \textbf{Lakebase (Postgres + pgvector)} for sub-10\,ms reads and exact-passage citations; \textbf{health-system interoperability} via FHIR so referrals flow into clinical systems; \textbf{real-time gold refresh} as new data arrives; \textbf{bring-your-own-data} so any NGO ingests its own export; \textbf{low-connectivity delivery} (SMS / offline-first) for field use; and extending multimodal intake from \emph{routing} to \emph{extraction}---reading capabilities off a facility board and structuring prescriptions and imaging through the same Model-Serving endpoint.
...
\bibitem{nfhs5} International Institute for Population Sciences (IIPS) and ICF. \emph{National Family Health Survey (NFHS-5), India, 2019--21}. Mumbai: IIPS, 2021.
\bibitem{spirtes2000} P.~Spirtes, C.~Glymour, and R.~Scheines. \emph{Causation, Prediction, and Search}, 2nd ed. MIT Press, 2000.
\bibitem{chernozhukov2018} V.~Chernozhukov, D.~Chetverikov, M.~Demirer, E.~Duflo, C.~Hansen, W.~Newey, and J.~Robins. Double/debiased machine learning for treatment and structural parameters. \emph{The Econometrics Journal}, 21(1):C1--C68, 2018.
\bibitem{rosenbaum1983} P.~R.~Rosenbaum and D.~B.~Rubin. The central role of the propensity score in observational studies for causal effects. \emph{Biometrika}, 70(1):41--55, 1983.
\bibitem{vanderweele2017} T.~J.~VanderWeele and P.~Ding. Sensitivity analysis in observational research: introducing the E-value. \emph{Annals of Internal Medicine}, 167(4):268--274, 2017.
...
\bibitem{lundberg2017} S.~M.~Lundberg and S.-I.~Lee. A unified approach to interpreting model predictions. In \emph{Advances in Neural Information Processing Systems (NeurIPS)}, 2017.
\bibitem{hastie1990} T.~Hastie and R.~Tibshirani. \emph{Generalized Additive Models}. Chapman \& Hall, 1990.
\bibitem{koenker1978} R.~Koenker and G.~Bassett. Regression quantiles. \emph{Econometrica}, 46(1):33--50, 1978.
\bibitem{gelman2007} A.~Gelman and J.~Hill. \emph{Data Analysis Using Regression and Multilevel/Hierarchical Models}. Cambridge University Press, 2007.
\bibitem{meinshausen2010} N.~Meinshausen and P.~B\"uhlmann. Stability selection. \emph{Journal of the Royal Statistical Society: Series B}, 72(4):417--473, 2010.
\bibitem{kipf2017} T.~N.~Kipf and M.~Welling. Semi-supervised classification with graph convolutional networks. In \emph{International Conference on Learning Representations (ICLR)}, 2017.
\bibitem{tobler1970} W.~R.~Tobler. A computer movie simulating urban growth in the Detroit region. \emph{Economic Geography}, 46:234--240, 1970.
\bibitem{moran1950} P.~A.~P.~Moran. Notes on continuous stochastic phenomena. \emph{Biometrika}, 37(1/2):17--23, 1950.
\bibitem{luo2003} W.~Luo and F.~Wang. Measures of spatial accessibility to health care in a GIS environment. \emph{Environment and Planning B: Planning and Design}, 30(6):865--884, 2003.
\end{thebibliography}

\appendix
\section{Glossary of methods (plain language)}
\small
\begin{tabular}{ll}
\toprule
Term & What it does for us\\
\midrule
Pearson $r$ & how tightly two things move together ($-1$ to $+1$); a hint, not a cause\\
PC algorithm & learns the graph of \emph{which variable could affect which} from the data\\
OLS $+$ fixed effects & estimates an effect while holding wealth and state constant\\
Double-ML & uses ML to strip out confounders, then reads the clean effect; robust to non-linear noise\\
PSM (propensity matching) & pairs similar ``treated'' vs ``control'' districts --- the observational stand-in for an A/B test\\
E-value & how strong an unknown confounder would need to be to overturn a result (bigger $=$ safer)\\
GAM & fits curved, non-linear dose--response instead of straight lines\\
Quantile regression & looks at the worst-off districts (the tails), not just the average\\
Multilevel model & stabilises noisy small-sample districts by borrowing strength across the group\\
Stability selection & keeps only predictors that survive repeated resampling (avoids flukes)\\
GNN (graph neural net) & geometric deep learning that learns from geography --- neighbours inform each other\\
Moran's $I$ & checks whether nearby districts look alike (spatial clustering); a diagnostic\\
ROC-AUC & how well a model separates classes; $1.0$ perfect, $0.5$ coin-flip\\
SHAP & shows how much each signal contributed to one prediction\\
2SFCA & accessibility score: reachable supply within a travel radius (roadmap)\\
DiD (difference-in-differences) & compares the \emph{change} in treated vs untreated districts over time (roadmap)\\
\bottomrule
\end{tabular}
