...
Roadmap: 2SFCA accessibility scoring~\cite{luo2003} to weight reachable supply within a travel radius; facility-placement optimization (where to build to maximally shrink the evidenced gaps); and a one-tap Google Maps \emph{Directions} link that turns a verdict into a next step (deeper last-mile integrations---in-app messaging and referral hand-off---remain future work). Multimodal intake is already live: the app accepts a photo---a facility board, a prescription, or the condition itself (an infected eye, a swollen limb)---and routes it to the right specialty using the \emph{same} Llama~4 Maverick Model-Serving endpoint as the text agents. The assistant also answers in eleven languages with text-to-speech.
